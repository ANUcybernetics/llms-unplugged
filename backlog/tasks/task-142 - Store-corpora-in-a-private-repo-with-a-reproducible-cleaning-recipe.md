...
- [ ] #2 Each corpus has a manifest entry recording its source URL, the sha256 of the raw file, and the cleaning steps applied to it
- [ ] #3 A script regenerates each cleaned corpus from its raw source, and its output matches the stored cleaned file byte for byte
- [ ] #4 Cleaning changes land as reviewable commits rather than in-place edits with no record
- [ ] #5 The llms-unplugged repo documents how to obtain the corpora, with data/ still gitignored
<!-- AC:END -->
