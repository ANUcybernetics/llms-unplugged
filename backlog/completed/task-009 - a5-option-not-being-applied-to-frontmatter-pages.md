...
Fixed by adding the `paper_size` parameter to the `set page()` calls in all
frontmatter functions in book.typ:
...
These functions now use `set page(paper_size, margin: ...)` instead of just
`set page(margin: ...)`, ensuring they respect the paper size input parameter
passed via `--input paper_size=a5`.
