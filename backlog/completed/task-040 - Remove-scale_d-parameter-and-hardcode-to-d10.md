...
Remove all references to scale_d throughout the codebase and hardcode to d10
(value 10). Since d10 is the best approach for scaling (easy to add more dice
for larger ranges), we no longer need the complexity of supporting arbitrary
dice sizes.
...
- Simplify scaling logic to always use 10^k-1 scaling (remove the match
  statement)
...
- [x] #1 No references to scale_d or dice_d remain in any .rs, .typ, or .md
      files
