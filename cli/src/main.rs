...
struct LedgerArgs {
    ...
    /// The counter colours the room has, as a JSON array of {"name": ...,
    /// "hex": ...} entries --- inline, or @path to a file. The default is the
    /// twelve in ledger-palette.json. The list is flat and the rows cycle
    /// through it `--columns` at a time, so twelve colours give a prefix
    /// three rows of distinct strips and eight give two; colours past the
    /// last whole row are dropped.
    #[arg(long, value_name = "JSON|@FILE")]
    palette: Option<String>,

    /// Follower cells on a row (default 4). A prefix with more followers than
    /// this continues onto the rows below, whose tally strips take the next
...
/// The palette a set prints in: the bundled default, a JSON array on the
/// command line, or one read from `@path`. Cut back to whole rows, since a
/// colour that no row reaches is a counter nobody can be asked to draw.
fn load_palette(spec: Option<&str>, columns: usize) -> Result<Vec<PaletteEntry>, CliError> {
    let mut palette = match spec {
        None => default_palette(),
        Some(spec) => {
            let json = match spec.strip_prefix('@') {
                Some(path) => fs::read_to_string(path).map_err(|err| {
                    CliError::InvalidArgs(format!("--palette cannot read {path}: {err}"))
                })?,
                None => spec.to_string(),
            };
            serde_json::from_str(&json).map_err(|err| {
                CliError::InvalidArgs(format!(
                    "--palette is not a JSON array of {{\"name\", \"hex\"}} entries: {err}"
                ))
            })?
        }
    };
    // Reported as the bad flag it is, rather than through the "error
...
fn run_ledger_command(args: &LedgerArgs) -> Result<(), CliError> {
    ...
    let counter_inputs: Vec<(String, String)> = inputs
        .iter()
        .filter(|(k, _)| k == "paper_size" || k == "json_path")
        .cloned()
        .collect();
    typst::compile_template(
        "ledger-counters.typ",
