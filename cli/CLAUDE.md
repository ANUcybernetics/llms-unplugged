...
- `lockup-light*.svg` - The brand lockup, generated by
  `website/scripts/generate-logo-svgs.ts` into `website/public/` and copied
  here so the templates reach it by relative path. The numbered variants are
  the five title tokens the 4x4 grid can spell --- the frames the favicon
  animates through --- and the sheet header cycles them, so a set spells the
  title out across every five sheets. `lockup-light.svg` is variant 1 again,
  and is what everything placing the mark once uses. Re-copy all of them after
  regenerating
