...
- `ledger-text.typ` - The training text as the tokeniser read it, written to
  `text.pdf` beside `ledger.pdf` whenever the set has a corpus: every token in
  a box with a running number beneath, punctuation in the sheets' symbol
  tile, dropped tokens dimmed and unnumbered. Reads the `text` field of
  `ledger.json` (one token list per document; see `text_documents` in
  `src/ledger.rs`)
