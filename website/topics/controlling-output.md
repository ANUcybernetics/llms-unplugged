---
title: Controlling Output
description:
  Learn how sampling strategies like temperature and truncation shape generated
  text without changing the underlying model.
---

# Controlling Output

![Hero image: Controlling Output](/assets/images/hero-controlling-output.avif)

Learn how sampling strategies like temperature and truncation shape generated
text without changing the underlying model.

## Lessons in this topic

### [Sampling](/lessons/sampling)

Experiment with temperature and truncation strategies to shape how your model
picks the next word.

## What you'll learn

After completing these lessons, you'll understand:

- How temperature controls randomness in text generation
- Why truncation strategies affect output quality
- The difference between changing the model vs. changing sampling
- How to get different styles of output from the same model
