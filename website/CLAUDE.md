...
`src/templates/index.ts` and the VFS in `typstCompiler.ts` --- which
`test/typstTemplateAssets.test.ts` checks; `test/ledgerTemplates.test.ts` then
compiles the ledger set end to end.
