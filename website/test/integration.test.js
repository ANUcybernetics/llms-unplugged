...
describe("topics and lessons", () => {
  it("generates topics index page", () => {
    const topicsIndexPath = join(siteDir, "topics", "index.html");
    expect(existsSync(topicsIndexPath)).toBe(true);
    const html = readFileSync(topicsIndexPath, "utf-8");
    expect(html).toContain("Topics");
    expect(html).toContain("Fundamentals");
    expect(html).toContain("Scaling up");
    expect(html).toContain("Controlling output");
  });

  it("generates individual topic pages", () => {
    const fundamentalsPath = join(
      siteDir,
      "topics",
      "fundamentals",
      "index.html",
    );
    const scalingPath = join(siteDir, "topics", "scaling-up", "index.html");
    expect(existsSync(fundamentalsPath)).toBe(true);
    expect(existsSync(scalingPath)).toBe(true);

    const fundamentalsHtml = readFileSync(fundamentalsPath, "utf-8");
    expect(fundamentalsHtml).toContain("Fundamentals");
    expect(fundamentalsHtml).toContain("Weighted Randomness");
    expect(fundamentalsHtml).toContain("Basic Training");
  });

  it("generates individual lesson pages with PDF links", () => {
