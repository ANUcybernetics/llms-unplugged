...
date: 2026-04-07
author: Ben Swift
description: Running LLMs Unplugged at the Tjabal Centre Autumn School with Luritja
  poet Matt Heffernan, using his poem Ngurrparringu as the training text
---
...
in both its Luritja and English versions. Participants did what _LLMs Unplugged_
...
generosity, and having the author in the room---as poet, translator, and
co-teacher---changed the shape of the whole morning.
...
Matt translated it for us in the moment:
...
> Maybe, in the old times, old men like me Stayed on Country, the Wind, the
> old men Sung Dreamtime, Not Sad anymore, but---ok, maybe I will hear the
> Wind, Wind is lonely---No.

Every word in the Luritja comes from Matt's poem; the model has just put them
in a different order.

![English word tokens: home, return, hear, the, sung, and, to, my](/assets/images/tjabal-workshop-03.avif)

There is nothing inherently English about a language model. The training data
determines the output language. Train on Luritja, get Luritja out. Train on a
Python codebase, get something that looks like Python. What goes in is what
comes out.

Another group trained on only the English version of _Ngurrparringu_ and pulled
out:

> Clearly anymore. I don't hear the wind, listening to the old men sat on the
> wind of my people, Warumpi

Every word Matt's, reshuffled. To close out the morning we all worked
together on one more generation, this time tipping _The Cat in the Hat_ into
the training mix alongside the English poem:

> Good Fun at all if you wish we sat in the mountains, we had something to
> Sing our Tjukurrpa, Cold

"Good fun" and "all if you wish" are Seuss; "mountains" and "Tjukurrpa" are
Matt. The model samples whatever tokens are in the bucket. Mix the training
data and you mix the output.

The corollary: if a language, a dialect, a community, or a knowledge tradition
isn't in the training data, it isn't in the outputs either. No representation
in, no representation out. When people say modern LLMs "know" English better
than other languages, they mean those LLMs were trained on orders of magnitude
more English text than anything else, and the tokenisers, evaluations and
benchmarks all reflect that choice. It's a property of what we fed them.
...
Tjabal Centre and the Autumn School participants for being up for something a
bit strange on a Tuesday morning. If you work with a language (or a
community, or a knowledge tradition) that's underrepresented in LLM training
data and want to try something like this, do [get in touch](/about#get-in-touch)---we'd
love to help.
