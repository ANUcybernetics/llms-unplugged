...
The cutouts variant of the [Training](/modules/training/) and
[Generation](/modules/generation/) lessons has had a serious refresh. If you
...
This gives you [weighted random sampling](/modules/weighted-randomness/) for
...
[grid version](/modules/training/) of the lesson. The cutouts skip the explicit
[weighted-sampling step](/modules/weighted-randomness/), which is the
...
table-spreads, where the grid bottlenecks at one teacher. The costs are a colour
printer (essential, since the colour coding is doing real work), scissors, and
more table than the grid wants. For primary-age students the cutouts are the
easier sell, and the weighted-sampling-without-the-maths trick still lands well
with adults.
...
time we wanted to spend on the [Generation lesson](/modules/generation/) itself.
...
card and a domino chain showing how each token becomes the next card's rightmost
prefix. The same tools page lets you generate cutouts from any text you like.

Both bigram (n=2) and trigram (n=3) configurations work, and you can push to n=4
or beyond. The [More Context lesson](/modules/more-context/) walks through the
trade-off: bigger n produces text that reads more like the source, at the cost
of a much bigger spread. Cutouts let you see this trade-off as physical paper
rather than an abstract claim.

If you can't print colour, or your students respond better to a tighter
board-style activity, the grid versions of [Training](/modules/training/) and
[Generation](/modules/generation/) are still here, still good. Both variants
