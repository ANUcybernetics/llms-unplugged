...
cutouts whose prefix box colour matches, choose one visually, and repeat using
...
The chain grows like dominoes: write a word, find a cutout whose prefix matches
it, write that cutout's token, then hunt for the next match. The spread already
encodes the weights---prefixes that occur more often have more cutouts on the
table, and your eye naturally lands on tokens that appear more often. So
...
1. **Pick a starting prefix**---choose any word that appears as a prefix on at
   least one cutout
2. **Find candidates**---scan the spread for cutouts whose prefix box matches
   your current word's colour, then verify the word itself matches before
   committing (two unrelated tokens can occasionally share a colour)
...
- scan for cutouts with `see` as the prefix (look for boxes in `see`'s
  colour)---both have `spot` as the token, so pick one and write down `spot`
...
more likely to produce that token: both `see` cutouts have `spot` as the
token, so generation from `see` always produces `spot`.
