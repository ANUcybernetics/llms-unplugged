...
together with the word it follows in the training text. The trick: if `spot`
follows `see` 40% of the time in the training text, then 40% of the cutouts
with `see` as the prefix will have `spot` as the token. Whichever matching
cutout your eye lands on, you've sampled in proportion to the original
distribution---no dice, no probability tables, the spread is doing the maths.
