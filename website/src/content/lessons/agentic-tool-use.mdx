...
2. Train or manually add counts so trigger words can appear in generation, and
   add "out" cutouts so each trigger transitions back to a common token (e.g.,
   `.`)
3. During generation, when you sample a trigger word:
   ...
   - continue generation from the trigger token's row (the "out" cutout), not
     from the result
...
paired with an "out" cutout that transitions to `.`
...
3. Sample → `GOOGLE` (trigger!)
   ...
   - Write down "exercise"
4. Continue from `GOOGLE` → "."
5. Sample → "I"
6. Sample → `ACTION` (trigger!)
   - Pause. Operator picks a student, who claps
   - Write down "clapped"
7. Continue from `ACTION` → "."

**Generated text:** "We should exercise. I clapped."

The operator's words ("exercise", "clapped") get written into the output, but
the next cutout always picks up from the _trigger token_, not the result. This
means tools can return literally anything---out-of-vocab words, phrases, even
sounds---without breaking the generation loop.
...
- add trigger words to "into" cutouts where they fit contextually (e.g.,
  `GOOGLE` after "a" or "the", `ACTION` after "we" or "I")
- pair every trigger with an "out" cutout that lands on a common token like
  `.`---this silently bridges the tool's result back into the model's
  vocabulary
- the operator's return can be _anything_: an out-of-vocab word, a phrase, even
  a noise. Continuation is keyed on the trigger token, not the result
...
The shape is always: something outside the model returns a word, the trigger's
out-cutout brings you back.
