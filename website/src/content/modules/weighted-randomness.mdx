...
Learn how to make <GlossaryTerm id="weighted-random-sampling">weighted random</GlossaryTerm> choices where some options are more likely than
others---an operation at the core of all generative AI.

![](../../assets/images/hero-weighted-randomness.avif)
...
Randomly choose from a fixed set of outcomes according to a given <GlossaryTerm id="probability-distribution">probability
...
Note: this is a warm-up. It's usually fine to start straight from
[Training](/modules/training/) and come back here only if students want more
detailed instruction about weighted random sampling.
...
- **generation mechanism**: every time an <GlossaryTerm id="llm">LLM</GlossaryTerm> produces a word, it's performing
  weighted random sampling from a probability distribution
- **probability distributions**: <GlossaryTerm id="neural-network">neural networks</GlossaryTerm> output probabilities for
  thousands of possible next <GlossaryTerm id="token">tokens</GlossaryTerm>; these probabilities become the "weights"
  for sampling
