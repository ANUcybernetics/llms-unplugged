...
an <GlossaryTerm id="agent">agent</GlossaryTerm> by giving it access to external
...
- a completed model from [Training](/modules/training/)
...
<GlossaryTerm id="tool-use">tool use</GlossaryTerm>---a model that can recognise
...
| `FRIEND`     | everyone texts a friend | text them the sentence so far     | first reply wins             |
...
friend the sentence so far---the first reply wins.
