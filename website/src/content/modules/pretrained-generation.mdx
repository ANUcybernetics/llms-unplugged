...
Use a (slightly larger) pre-trained model booklet to generate new text through <GlossaryTerm id="weighted-random-sampling">weighted random sampling</GlossaryTerm>
.
...
- a pre-trained model <GlossaryTerm id="booklet">booklet</GlossaryTerm>
...
The [Materials](/tools/#booklets-for-dice-generation) page has ready-to-print
...
like your hand-built <GlossaryTerm id="bigram-model">bigram model</GlossaryTerm>: look up the current word, roll dice, choose
...
3. Roll your d10(s):
   - If the entry shows diamonds (e.g. ♦♦), roll one d10 per diamond
     and read the digits as one number.
...
- **Multiple d10s:** Current word `the` with two diamonds (♦♦) and options
...
- what <GlossaryTerm id="vocabulary">vocabulary</GlossaryTerm> size does the booklet model have compared to your hand-built
  model?
...
- **<GlossaryTerm id="pre-training">pre-training</GlossaryTerm>**: companies train massive models on huge text <GlossaryTerm id="training-data">corpora</GlossaryTerm> (like
...
booklet captures thousands of training examples in a portable format, just as <GlossaryTerm id="neural-network">neural networks</GlossaryTerm> compress
training data into billions of <GlossaryTerm id="parameters">parameters</GlossaryTerm>.
