...
This module involves rolling dice to sample from weighted probability
distributions. If your students need extra support with this concept, consider
running the [Weighted Randomness](/modules/weighted-randomness/) module first.

</Callout>

</Variant>

Use a pre-trained (hand-built) <GlossaryTerm id="bigram-model">bigram model</GlossaryTerm> to generate new text through <GlossaryTerm id="weighted-random-sampling">weighted random sampling</GlossaryTerm>
.
...
- your completed bigram model from [Training](/modules/training/)
...
- the trained cutouts spread from [Training](/modules/training/) (already laid
  out on a table)
- pen and paper for writing down the generated text

The [Materials](/tools/#cutouts) page has ready-to-print token cutouts for several
texts if you haven't done the Training module yet. Each PDF starts with a
...
A <GlossaryTerm id="language-model">language model</GlossaryTerm> proposes several possible next words along with how likely each
...
The chain grows like dominoes: write a word, find a <GlossaryTerm id="cutouts">cutout</GlossaryTerm> whose previous-word
...
1. Choose a starting word from the first column of your <GlossaryTerm id="grid">grid</GlossaryTerm>.
2. Look at that word's row to find all possible next words and their counts.
3. Roll dice weighted by the counts (see
   [Weighted Randomness](/modules/weighted-randomness/)).
...
Using the same bigram model from the example in [Training](/modules/training/):
...
Using the cutouts spread from the example in [Training](/modules/training/):
...
module's optional extension), generation becomes faster: step 2 is now "find
...
- in as much detail as you can, explain what happens after typing something into the <GlossaryTerm id="chatbot">ChatGPT or Claude</GlossaryTerm> prompt box to produce the answer you get back
...
- **"Every row only has one tally mark---there's nothing to roll for."** If the
  group didn't get very far in [Training](/modules/training/) and no row has more
  ...
  really just an example of using [Synthetic Data](/modules/synthetic-data/).
...
- **"Every previous word only has one matching cutout---there's nothing
  ...
  [Synthetic Data](/modules/synthetic-data/).
...
This <GlossaryTerm id="generation">generation</GlossaryTerm> process is identical to how current <GlossaryTerm id="llm">LLMs</GlossaryTerm> produce text:
...
- **<GlossaryTerm id="probability-distribution">probability distribution</GlossaryTerm>**: <GlossaryTerm id="neural-network">neural network</GlossaryTerm> outputs probabilities for all
  50,000+ possible next <GlossaryTerm id="token">tokens</GlossaryTerm>
