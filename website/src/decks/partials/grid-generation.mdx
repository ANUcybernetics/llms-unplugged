...
<StaticGeneration
  ...
  vocabulary={EXAMPLE_VOCAB}
  sequence={EXAMPLE_GENERATION}
...
## Generation: from `joey`

```notes
two options in this row (`hop` and `,`) with equal tallies --- highlight both, then roll
```
...
## How the die chooses: `joey`

```notes
equal tallies, so the ten d10 faces split down the middle --- `hop` gets 0-4 and `,` gets 5-9, reading the row's cells left to right; same apportionment the printed booklets use. Pause here before rolling.
```
...
```notes
another two-option row (`hop` and `joey`), still equal --- both highlighted, then roll
```
...
```notes
rolled a 2, which lands in the `hop` band
```
...
write down a starting token, then repeat:

1. **find the row** for the last token you wrote
2. **share the ten faces** of the d10 across that row's cells, by their tallies
3. **roll** --- the band it lands in names the next token (one option? no roll)
4. **write it down**, and look up its row next

<Timer minutes={10} overlay />

```notes
This is the whole loop on one slide --- leave it up while they work. Groups generate from their own grid, rolling for each choice. 10 min, circulate. Where a row has only one option, no roll needed.
```
