...
**same recipe, bigger model**: yours learnt from a few sentences, this one learnt
from a whole novel
...
Worth flagging what to watch for, so they read their output rather than just producing it: it'll sound more like real prose, and it still won't be about anything. That's the reflection question at the end of the section.
...
it reads better---but is it **saying** anything?

{/* notes:
Second reflection beat. More data bought fluency and nothing else: the booklet's output sounds like prose because Frankenstein sounds like prose, and there is still nobody home. ~2 min, shorter than the last one.

The connection worth drawing out, if the room doesn't get there: this is the gap between text that reads well and text that's about something, and it's the same gap they've felt with a real model that answered confidently and wrongly. Fluency was never evidence of understanding --- they've now watched fluency get manufactured out of tally marks.

Take one or two answers and move; the scaling-up section is where this gets paid off properly.
*/}
