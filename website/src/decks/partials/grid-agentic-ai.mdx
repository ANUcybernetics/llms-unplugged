...
<li><strong>text the sentence so far</strong> (everything since the last full stop) to 3 friends</li>
...
**tool call**: text the sentence so far to 3 friends, and the whole first reply
...
text "the cat sat" (the whole sentence so far) to 3 friends;
...
{/* notes:
Generate from your model --- on punctuation, text the sentence so far to 3 friends and splice in the whole first reply. 10 min --- texting latency is real, so keep groups moving; if a reply hasn't landed by the next punctuation token, back-fill it later. Circulate.
*/}
