...
**tool call**: text the previous word to 3 friends, and the whole first reply
goes into your text---followed by the punctuation you rolled
...
{/* notes:
Walk it slowly: the punctuation is the trigger, the previous word is the tool input, the whole first reply is the tool result. The punctuation still gets written --- the tool result slots in just before it. If asked why we don't continue from the friend's reply: those words probably aren't in your model's vocabulary, so resuming from the punctuation token is the cleanest way to keep generating.
*/}
...
{/* notes:
Generate from your model --- on punctuation, text the previous word to 3 friends and splice in the whole first reply. 10 min --- texting latency is real, so keep groups moving; if a reply hasn't landed by the next punctuation token, back-fill it later. Circulate.
*/}
