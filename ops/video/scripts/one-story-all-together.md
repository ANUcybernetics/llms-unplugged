# One story, all together (ledger)

**Key idea:** pool everyone's models and the class can say things no single
group's model could.

**Plays:** How AI writes stories (ledger), start of part 6 (One story, all
together). The five `ROW_THE_*` rows are the five school-day texts: bell, bus,
dog, rain, volcano.

## Beat sheet

1. hook: every group trained on a different text. Nobody has _volcano_ except
   the volcano group. Nobody has _bus_ except the bus group. But the room has
   all of them
2. the same word, five rows: what came after _the_ in your text? Five honest
   counts, five different answers, none of them wrong
3. the claim: together the class holds a bigger model than any one group
4. how: call out the word we're on; hands up if your sheets have a row for it;
   pick a group; they read out their colours and tallies; the cup up front
   draws; their word is the class's next word
5. two steps per word, same as always: the show of hands picks the group, the
   cup picks the word
6. the fun moment: a word only one group has lands in the class story---the
   class model says things most groups never saw
7. connection: this is what a big model is---many texts pooled into one set of
   counts, so it can say things no single text could. And a bigger model still
   runs one word at a time
8. CTA: butchers paper up the front. First word: _the_. Hands up

## Script

**BEN (TC):** Every group in this room trained on a different text. Nobody has
_volcano_ except the volcano group. Nobody has _bus_ except the bus group.

**EDDIE (TC):** But the room has all of them.

_Visual: the five ROW_THE rows stacked---bell, bus, dog, rain, volcano._

**EDDIE (VO):** Here's the same word, _the_, from five different models. Five
honest counts of five different texts, five different answers. None of them is
wrong. And read down the followers and you can name the text: bell and key, bus
and air, roof and sun.

**BEN (VO):** Together, the class holds a bigger model than any one group. So
let's generate from it.

_Visual: the class-story loop---a word called, hands up, a group picked, its row
shown, the cup up front filling and drawing, the word landing on butchers
paper._

**EDDIE (VO):** Two steps per word, same as always. Call out the word we're on.
Hands up if your sheets have a row for it---the show of hands picks the group.
That group reads out its colours and tallies: three red, one blue, one green,
one yellow. The counters go in the cup up the front, one comes out, and that
group reads the word beside the colour. The cup picks the word.

_Visual: LedgerRow for "the" (volcano), blue drawn, "volcano" appearing on the
class story._

**BEN (VO):** Blue. That group's blue word is _volcano_. It goes on the class
story, and _volcano_ is the next word we hunt for.

**EDDIE:** And that's the fun of it. A word only one group ever saw just landed
in everyone's story. The class model can say things most of the room's models
never could.

**BEN:** Which is what a big model is. Many texts, pooled into one set of
counts, so it can say things no single text could. And notice: a bigger model
still runs one word at a time.

**EDDIE (TC):** Butchers paper up the front. First word: _the_. Hands up.
